      ...
      <div className="page-hero" style={{ minHeight: 'auto', padding: '24px 0 40px' }}>
        <div className="container">
          ...
          <div className="content-box">
            ...
            <p style={{ marginBottom: 20 }}><Link href="/edit-account" style={{ fontSize: '.82rem', color: '#2563eb' }}>edit your password and account details.</Link></p>
            ...
            <table className="account-table" style={{ marginBottom: 24 }}>
              ...
              <tbody>
                {orders.map((o, i) => (
                  <tr key={i}>
                    <td><Link href="/order-details">{o.id}</Link></td>
                    <td>{o.date}</td>
                    <td>{o.status}</td>
                    <td>{o.total}</td>
                    <td><Link href="/order-details" className="btn btn-red btn-sm">VIEW ORDERS</Link></td>
                  </tr>
                ))}
              </tbody>
            </table>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: addrCols, gap: 20 }}>
              {[{ label: 'Billing address', link: '/edit-billing' }, { label: 'Shipping address', link: '/edit-shipping' }].map((a, i) => (
                <div key={i}>
                  ...
                  <div style={{ marginTop: 10 }}><Link href={a.link} className="btn btn-red btn-sm">EDIT ADDRESSES</Link></div>
                </div>
              ))}
            </div>
          </div>
