        ...
        <div className="container">
          ...
          <div className="content-box">
            ...
            <table className="account-table" style={{ marginBottom: 20 }}>
              ...
              <tbody>
                {orderItems.map((item, i) => (
                  <tr key={i}><td><Link href="/product" style={{ color: '#2563eb' }}>{item.name}</Link></td><td>{item.qty}</td><td style={{ textAlign: 'right' }}>{item.total}</td></tr>
                ))}
              </tbody>
            </table>
            ...
            <div style={{ marginTop: 20, display: 'flex', gap: 10, flexWrap: 'wrap' }}>
              <Link href="/" className="btn btn-gray">CONTINUE SHOPPING</Link>
              <Link href="/my-account" className="btn btn-red">MY ACCOUNT</Link>
            </div>
          </div>
        </div>
      </div>
